...
import { FileText, Loader2, Send, Sparkles, Upload, LogOut, MessageSquare, Plus, Square, Trash2, BookOpen, RefreshCw, AlertCircle } from "lucide-react";
...
type Doc = { id: string; title: string; status: string; page_count: number; error?: string | null; file_path?: string };
    ...
    <div className="min-h-screen flex">
      ...
      <aside className="w-72 border-r border-border/40 flex flex-col p-4 gap-3">
        ...
        <div className="flex-1 overflow-y-auto space-y-1">
          ...
          {docs.map((d) => (
            <div key={d.id}
              className={`group w-full rounded-xl px-3 py-2.5 flex items-start gap-2 transition-all ${activeDoc?.id === d.id ? "glass-strong ring-aurora" : "hover:bg-white/5"}`}>
              <button onClick={() => setActiveDoc(d)} className="flex items-start gap-2 flex-1 min-w-0 text-left">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {d.status === "ready" ? `${d.page_count} pages` :
                     ...
                     <span className="inline-flex items-center gap-1 text-destructive" title={d.error ?? "Failed"}><AlertCircle className="w-3 h-3" /> Failed</span>}
                  </div>
                </div>
              </button>
              <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                {d.status === "failed" && (
                  <button onClick={() => retryDoc(d)} title="Retry processing"
